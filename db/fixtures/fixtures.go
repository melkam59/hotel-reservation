...
func AddBooking(store *db.Store, uid, rid int, from, till time.Time) *types.Booking {
...
func AddRoom(store *db.Store, size string, ss bool, price float64, hid int) *types.Room {
	room := &types.Room{
		Size:    size,
		Seaside: ss,
		Price:   price,
...
func AddHotel(store *db.Store, name string, loc string, rating int, rooms []int) *types.Hotel {
	var roomIDS = rooms
	if rooms == nil {
		roomIDS = []int{}
	}
