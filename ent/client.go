...
import (
	...
	"github.com/fulltimegodev/hotel-reservation/ent/migrate"
...
func (c *BookingClient) QueryRoom(_m *Booking) *RoomQuery {
...
// UpdateOneID returns an update builder for the given id.
func (c *HotelClient) UpdateOneID(id int) *HotelUpdateOne {
...
// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *HotelClient) DeleteOneID(id int) *HotelDeleteOne {
...
func (c *HotelClient) Get(ctx context.Context, id int) (*Hotel, error) {
	return c.Query().Where(hotel.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *HotelClient) GetX(ctx context.Context, id int) *Hotel {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// QueryRooms queries the rooms edge of a Hotel.
func (c *HotelClient) QueryRooms(_m *Hotel) *RoomQuery {
	query := (&RoomClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(hotel.Table, hotel.FieldID, id),
			sqlgraph.To(room.Table, room.FieldID),
...
// QueryHotel queries the hotel edge of a Room.
func (c *RoomClient) QueryHotel(_m *Room) *HotelQuery {
	query := (&HotelClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
...
func (c *RoomClient) QueryBookings(_m *Room) *BookingQuery {
...
type (
	hooks struct {
		Booking, Hotel, Room, User []ent.Hook
	}
	inters struct {
		Booking, Hotel, Room, User []ent.Interceptor
	}
)
