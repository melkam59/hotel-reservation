...
type Hotel struct {
	...
	// The values are being populated by the HotelQuery when eager-loading is set.
	Edges        HotelEdges `json:"edges"`
	selectValues sql.SelectValues
}

// HotelEdges holds the relations/edges for other nodes in the graph.
type HotelEdges struct {
...
func (e HotelEdges) RoomsOrErr() ([]*Room, error) {
	if e.loadedTypes[0] {
		return e.Rooms, nil
	}
...
func (*Hotel) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case hotel.FieldID, hotel.FieldRating:
			values[i] = new(sql.NullInt64)
		case hotel.FieldName, hotel.FieldLocation:
...
// QueryRooms queries the "rooms" edge of the Hotel entity.
func (_m *Hotel) QueryRooms() *RoomQuery {
	return NewHotelClient(_m.config).QueryRooms(_m)
}
